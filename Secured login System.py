...
firebase = firebase.FirebaseApplication("https://d-login-system-8db02-default-rtdb.firebaseio.com/",None)
...
def login():
    ...
    global login_username_enrty
    username = login_username_enrty.get()
    pasword = login_password_entry.get()

    encrypted_password = hashlib.md5(password.encode())
    hexadecimal_password = encrypted-passwword.hexidigest()
    get_password = fiebase.get('/Users', username)
    ...
    if(get_password != None):
        if(get_password == hexadecimal_password):
            ...
            messagebox.showinfo("Information", "User not registerd! \nto login get yourself registerd first")
def delete():
    global login_username_entry
    ...
    encrypted_password = hashlib.md5(password.encode())
    hexadecimal_password = encrypted_password.hexidigest()
    get_password = firebase.get('/Users', username)

    print(hexadecimal_password)
    if(get_password != None):
        if(get_password == hexadecimal_password):
            firebase.delete('/Users',username)
            messagebox.showinfo("Information", "Successfully Account Delete")
        else:
                messagebox.showinfo("Information", "Please check your Password")
    else:
     messagebox.showinfo("Information", "User not registered! \nTo login get youurself registered first")
...
def register():
    ...
    print(hexadecimal_password)
    put_data = firebase.put("/Users", username, hexadcimal_password)
    messagebox.showinfo("Information", "Successfully registered")

def login_window():
    global login_userrname_entry
    global login_password_entry
...
def delete_window():
    ...
    log_heading_label = Label(login_window, text="Delete Account Form" , font = 'arial 18 bold', bg ='#FCF6B1')
    log_heading_label.place(relx=0.5,rely=0.2, anchor=CENTER)
    ...
    btn_login = Button(login_window, text="Delete" , font = 'arial 13 bold' , bg="#2D1E2F", fg="white", command=delete, relief=FLAT)
...
btn_delete_window = Button(registration_window, text="Delete" , font = 'arial 10 bold' , bg="#2D1E2F", fg="white", command=delete_window, relief=FLAT)
btn_delete_window.place(relx=0.9,rely=0.16, anchor=CENTER)
